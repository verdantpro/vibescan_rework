    ...
    <div className="page wrap sig">
      ...
      <div className="sig-main">
        ...
        <aside className="sig-info panel panel-pad">
          ...
          <Field label="OPERATOR" value={s.whois} />
          <Field label="ORIGIN" value={geo ? [geo.city, geo.region, geo.country].filter(Boolean).join(", ") : null} />
          <Field label="COORD" value={geo ? `${geo.lat.toFixed(4)}, ${geo.lon.toFixed(4)}` : null} />
          <Field label="COUNTRY" value={geo?.country_iso} />
          <Field label="CERT CN" value={s.cert_cn} />
          <Field label="pHASH" value={s.screenshot_phash} />
          <Field label="DOM HASH" value={s.dom_hash} />
          <Field label="CAPTURE" value={s.capture_hash ? `${s.capture_hash}.${s.capture_ext}` : null} />
          <Field label="SUBMITTER" value={d.anon ? "anonymous" : d.submitted_by} />
